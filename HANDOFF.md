...
- **Icon brightened to orange** `#E08A33` (was muted gold `#B07F2C`) in `scripts/make-icon.mjs` and the `index.html` favicon; `build/icon.png`/`.ico` regenerated. The center mark is now a **simple cream open-book line drawing** (two outlined portrait pages with a slight splay meeting at the spine) instead of the old ring; still pure SDF math, still legible at 16px.
