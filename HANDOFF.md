...
- **Icon is now the user's own brand mark.** The user supplied a logo package (kept at `design/logo/`, read its README): an ink `#0B1512` squircle (rx 24%) with a green `#12B76A` open-book stroke, from `design/logo/assets/icon-book-dark-squircle.svg`. `scripts/make-icon.mjs` reproduces that SVG exactly with dependency-free SDF math (bezier-flattened polyline strokes), so `npm run dist` regenerates crisp PNG/ICO at every size; the `index.html` favicon inlines the same SVG. History for context: the ring was briefly brightened orange, a Claude-drawn book was tried and rejected, then the user delivered this official mark — it wins. The package also contains a `life.org` wordmark direction (Outfit 700, green period) not yet used in the app; a future branding pass could adopt it for the sidebar.
