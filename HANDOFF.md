...
- **The brand is the `life.org` WORDMARK, everywhere — final, explicit user direction.** The user's logo package lives at `design/logo/` (read its README). They rejected every glyph option (the ring, a Claude-drawn book, and their own package's book icons) and chose the type-only wordmark: lowercase `life` + geometric green period (`#12B76A`, 0.18em, baseline) + `org`, Outfit 700, `-0.02em`, ink `#0B1512` (light) / `#F2F5F3` (dark). They were told it is illegible below 48 px and answered "I don't care" — do NOT swap a glyph back in.
  - **App icon**: `scripts/make-icon.mjs` now rasterizes the wordmark ramp (exact per-size specs from the design) in headless Chromium; text can't be drawn dependency-free, so `build/icon.png` / `icon.ico` / `icon-32.png` are **committed artifacts** and `npm run dist` no longer regenerates them (the `icon` npm script is manual: `npm i --no-save playwright-core` + a Chromium binary).
  - **In app**: Outfit 700 latin is bundled (`src/assets/fonts/outfit-latin-700.woff2`, OFL), `Wordmark` component in `components/ui.tsx` (ink via `--wordmark-ink` token), used in the sidebar, the desktop title bar, and onboarding. The favicon is the rendered 32 px mark inlined in `index.html`. The brand green appears only in the wordmark's dot; the app palette is otherwise untouched.
- **Installer delivery: the `release-drop` branch.** The build workflow (permissions: `contents: write`) force-pushes the freshly built `Life.Org Setup 1.0.0.exe` onto the orphan `release-drop` branch after every Windows build. The user downloads it via the file's "Download raw file" button on GitHub; sessions can fetch it over authenticated git (`git show "origin/release-drop:Life.Org Setup 1.0.0.exe"`). This exists because Actions artifacts expire after 90 days and cannot be fetched from inside a cloud session, and the installer (~96 MB, under GitHub's 100 MB blob limit) cannot be attached to chat (30 MiB cap). Do not commit installers to normal branches.
- **Brain-dump entries are deletable.** Each captured row has a hover-quiet `×` guarded by a themed confirm (per the user: ask every time). `store.deleteDumpItem(id)` removes the entry and exactly one matching vault copy (matched by tag + snippet, since capture assigns no shared id), through `updateWithHistory` so the day's `captured` count stays honest.
- **Anime.js wave (same day, user-directed): subtle-premium motion, palette untouched.** `animejs` v4 is now a dependency alongside `motion`; the user explicitly requested its `createTimeline`, `createDraggable`, and `svg.createMotionPath` APIs, and all three are in use:
  - `src/lib/anim.ts`: `reducedMotion()` guard + `enterSection()`, a `createTimeline` stagger that raises/fades each module's blocks on every module switch. Driven by `ModuleFrame` in `App.tsx` (layout effect hides children pre-paint, inline styles cleared on complete so hover transitions survive). `.fade-up` CSS is now opacity-only so it does not double the movement.
  - `FollowerChart`: on mount the line draws itself (`svg.createDrawable`) while an accent dot rides the tip via `svg.createMotionPath`, settling on the newest reading. Skips cleanly under reduced motion.
  - Dashboard schedule reorder replaced HTML5 drag-and-drop with `createDraggable` (trigger = the `⋮⋮` grip, y-axis only, contained to the list). The row lifts (`.drag-lift`), the nearest slot shows the `.drag-over` line, release resets the transform and calls `reorderSchedule`. Draggables are rebuilt whenever the schedule's id order changes and `revert()`ed on cleanup. Verified end to end in Chromium: order swap persists.
  - CSS polish: `.goal-card`/`.task-row` hover lift 1px, `.ghost-btn`/`.accent-btn`/`.danger-btn` settle to `scale(0.97)` on press.
  - Rule for future motion work: subtle and premium, nothing bounces, never touch the palette, always respect `prefers-reduced-motion`.
...
**Jul 10 final pass: `scripts/verify.js` modernized and green, 49/49.** The harness now asserts the honest-offline contract everywhere an engine feature used to return canned stub output: replan/cascade/coach/weekly/connections/patterns/strategist/finance each REFUSE with the "This needs the engine" toast and change nothing; reflections save offline with `output: null` and no read; preference learning and chat summarization stay quietly empty offline; brain-dump classification still works and captured entries delete only behind the themed confirm (both dump and vault copies). The Goal Center remove selector is `button:text-is("Remove")` after the redesign. Run it with a dev server on :5173 plus `npm i --no-save playwright-core`. Anything below about "40 checks" predates this.

**The older multi-dimension sweep is half done.** `scripts/debug-sweep.workflow.js` (5 review dimensions, then 3 adversarial verifiers per finding, run via the Workflow tool) was launched over the full codebase; the session ended after only the **persistence** and **state-react** reviewers finished. Their 20 findings were manually adjudicated and **the real ones are FIXED and verified** (see "Fixed from the sweep" below). Three dimensions **never ran** and are the next session's first job:
